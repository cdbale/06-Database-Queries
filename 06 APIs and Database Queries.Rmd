---
title: "APIs and Database Queries"
...
Remember that you grade your own exercises using the rubric specified in each posted exercise solution, adding comments (using the comment feature for Word documents) where your solution differs from the posted solution.
...
Patagonia is very impressed with what we've been able to do with their in-house data. They have decided to open up additional data sources for us so we can expand our analyses. Patagonia recently used Qualtrics to administer a survey to some of their customers. This survey data will need to be accessed using an API. The second additional source contains information on the revenue from each customer. This data is stored in a SQL data base. 

- Has anyone used or accessed a database before?
- What about an API?
...
**APIs** (Application Programming Interfaces) are a *general* way to access any server that provide permission to access data along with securely controlling what and how much data you can access.
...
To use an API you should use an **API client** specific to the server. There are a plethora of R packages that provide API clients for specific servers (e.g., {twitteR}, {Rfacebook}, {RGoogleAnalytics}, {WikipediR}, {qualtRics}). API clients differ, but they typically require at least:
...
Install and load the {qualtRics} package so that we can access the survey data.
...
We can import data using the survey ID.
...
## Let's AVOID Reconnecting to the API - Write Your Data to your Hard Drive

```{r, eval = FALSE}

# This code "writes" the data, which means it gets saved locally on your 
# computer instead of just in this R session.

# Write it as a CSV to use outside of R.
survey_data |> 
  write_csv("example_qualtrics_survey.csv")
```

## Your Turn!

Fetch the "Hammocks and Life" survey and write it to your local hard drive. Hint: they survey ID is `SV_73xYIxZuxsqAx6Z`.

## Expected Survey

```{r, echo=FALSE, results=FALSE}
library(qualtRics)
hammock_survey_data <- fetch_survey("SV_73xYIxZuxsqAx6Z")
```
...
hammock_survey_data |>
  write_csv("example_hammock_survey.csv")
```

## Databases and SQL Disclaimer

Unfortunately, AWS is being difficult and was not able to set up an example data base for us to use.

HOWEVER...

The process (and principles) are extremely similar to accessing an API. So, we'll look at example code and call it good!
...
Similar to APIs, once we have the data we need we can write it locally.
...
Patagonia currently measures the overall satisfaction level of customers and the likelihood that customers would recommend the store. While these measures have been frequently collected by the company, they don't have a clear idea of whether they are related to outcomes the company truly cares about, such as revenue. Our task is to investigate these survey results and to determine whether Patagonia should continue to collect these survey responses (and analyze them further) or look for other ways of assessing the customer experience.

(Let's treat this as a mini-project).

## Wrapping Up

*Summary*

- Discussed APIs and databases.
...
Patagonia has requested that we re-do our RFM analysis now that we have access to the revenue per month per customer. In RStudio, create a new Quarto document and do the following.
